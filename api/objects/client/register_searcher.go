...
// RegisterSearcher registers the caller as a searcher provider and blocks until
// the node answers with the lease it granted.
//
...
func (client *Client) RegisterSearcher(ctx *astral.Context, duration astral.Duration) (lease *objects.RegistrationLease, err error) {
	...
	ch, err := client.queryCh(ctx, objects.MethodRegisterSearcher, args)
	if err != nil {
		return
	}
	defer ch.Close()
...
func RegisterSearcher(ctx *astral.Context, duration astral.Duration) (*objects.RegistrationLease, error) {
	return Default().RegisterSearcher(ctx, duration)
}
