...
// RegisterDescriber registers the caller as a describer provider and blocks
// until the node answers with the lease it granted.
//
...
func (client *Client) RegisterDescriber(ctx *astral.Context, duration astral.Duration) (lease *objects.RegistrationLease, err error) {
	...
	ch, err := client.queryCh(ctx, objects.MethodRegisterDescriber, args)
	if err != nil {
		return
	}
	defer ch.Close()
...
func RegisterDescriber(ctx *astral.Context, duration astral.Duration) (*objects.RegistrationLease, error) {
	return Default().RegisterDescriber(ctx, duration)
}
